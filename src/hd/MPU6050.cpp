...
namespace qing {
	...
	MPU6050::MPU6050(I2C& i2c): i2c(i2c)	/* 初始化 MPU6050 */
	{
		i2c.lock();
		i2c.setAddr(MPU6050_ADDR_0);
		i2c.writeReg(MPU6050_电源管理寄存器1,    MPU6050_电源管理_非睡眠模式);	/* 退出睡眠 */
		i2c.writeReg(MPU6050_陀螺仪配置寄存器,   MPU6050_陀螺仪量程_正负500度每秒);	/* ±500°/s */
		i2c.writeReg(MPU6050_加速度计配置寄存器, MPU6050_加速度计量程_正负8g);	/*  ±8g */
		flag = true;
		i2c.unlock();
		usleep(100000);	/* 等待稳定 */
	}

	int16_t MPU6050::read16bitReg(uint8_t high, uint8_t low) {	/* 读取 16 位传感器数据（带符号） */
		return (i2c.readReg(high) << 8) | i2c.readReg(low);
	}

	MPU6050::sensorData MPU6050::readSensor() {	/* 获取原始MPU6050传感器数据 */
		sensorData data;
		i2c.lock();
		i2c.setAddr(MPU6050_ADDR_0);
	...
	void MPU6050::calcAngle(const MPU6050::sensorData& data, float& pitch, float& roll)
	{
		/* - ±2g → 16384 LSB/g
		 - ±4g → 8192 LSB/g
		 - ±8g → 4096 LSB/g
